...
const PremiumManagement = ({ onBack }) => {
  ...
  const fetchPremiumUsers = async (page = pagination.currentPage) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/admin/memberships?page=${page}&limit=${pagination.limit}`, {
        headers: {
  ...
  const fetchStats = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('${API_BASE_URL}/api/admin/memberships/stats', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const updateMembershipStatus = async (userId, role) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/api/admin/memberships/${userId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const extendMembership = async (userId, role) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/api/admin/memberships/${userId}/extend`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Avatar className="w-12 h-12 border border-amber-500/30">
                          <AvatarImage 
                            src={membership.user?.avatar ? `${API_BASE_URL}${membership.user.avatar}` : '/uploads/avatar/default.png'} 
