...
app.post('/api/upload', protect, upload.single('image'), async (req, res) => {
  try {
    ...
    // ถ้ามี user ID ใน request (จาก authentication)
      ...
      try {
        ...
        if (user) {
          const photoData = {
            ...
            order: user.photos.length // เรียงลำดับตามจำนวนรูปที่มีอยู่
          };
          
          user.photos.push(photoData);
          await user.save();
          
          console.log(`📷 Image uploaded and saved to database for user: ${req.user.username}`);
          console.log(`📁 File: ${req.file.filename}`);
          console.log(`👤 User: ${req.user.username} (${req.user.id})`);
        }
      } catch (dbError) {
        console.error('❌ Database error:', dbError);
        // ไม่ return error เพราะไฟล์อัพโหลดสำเร็จแล้ว
      }
    }
...
// เพิ่ม admin message routes
const adminMessageRoutes = require('./routes/adminMessages');
app.use('/api/admin/messages', adminMessageRoutes);

// เพิ่ม admin activity routes
const adminActivityRoutes = require('./routes/adminActivities');
...
// เพิ่ม superstars routes
const superstarsRoutes = require('./routes/superstars');
app.use('/api/superstars', superstarsRoutes);
